...
# инициализация:
file_name = 'spindle_controller_setup_1'
...
bl_compensation_y = 4
# последнее значение z_compensation: 45000
# z_compensation = 1000 это значит поднять на 1/100 мм
z_compensation = 0      # все координаты по Z поднимаются на данный коэффициент. Связано это с тем, что в данной
...
# когда нужны неизмененные координаты Z, мы устанавливаем z_compensation = 0
        ...
        for unit in list_line:                  # перебираем все юниты в текущей строке и ищем, где есть M коды
            if unit.startswith('M'):                # в текущем юните нашли какой-то M код
                ...
                if cur_m_code == 0:                     # обрабатываем код M0 "программируемый останов"
                    ...
                    y_cur = 0
                    z_cur = 0
                if cur_m_code == 3:                     # обрабатываем код М3 включение шпинделя (по часовой стрелке)
                    arr_prog.append("M3")
                if cur_m_code == 5:                     # обрабатываем код M5 выключение шпинделя
                    arr_prog.append("M5")
                if cur_m_code == 2:
                    arr_prog.append("M2")
