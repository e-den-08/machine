            ...
            if unit.startswith('M'):                # в текущем юните нашли какой-то M код
                ...
                if cur_m_code == 6:                     # обрабатываем код M6 (M06) смена инструмента
                    ...
                    if tool_num != 0:                   # T0 в конце программы записывать не надо
                        ...
                        x_cur = 0
                        y_cur = 0
                        # когда мы нажимаем тумблер "Вернуться к референтной точке (нулевой точке детали), шпиндель
                        # по оси Z не доходит до референтной точки 5мм. (выста подстроечной параллельки). Поэтому
