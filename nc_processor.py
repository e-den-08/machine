...
bl_compensation_x = 2       # длина компенсации люфта в шагах (проверено микрометром)
bl_compensation_y = 1
...
# --------------------------------------------------------------------------------------------
# |     1/2   |        400 имп.      |      200 имп.    |     0.005мм      |       500       |
# -------------------------------------------------------------------------------------------
...
for line in gc_list:
    if line.startswith('('):        # если текущая строка - это комментарий
        continue                        # значит мы её вообще никак не обрабатываем и переходим к следующей
            ...
            if unit.startswith('M'):                # в текущем юните нашли какой-то M код
                ...
                if cur_m_code == 6:                     # обрабатываем код M6 (M06) смена инструмента
                    ...
                    if tool_num != 0:                   # T0 в конце программы записывать не надо
                        ...
                        x_cur = 0
                        y_cur = 0
                        # когда мы нажимаем тумблер "Вернуться к референтной точке (тнулевой точке детали), шпиндель
                        # по оси Z не доходит до референтной точки 5мм. (выста подстроечной параллельки). По этому
                        # эту высоту и нужно будет компенсировать в первом же кадре, содержащем движение по Z
                        z_cur = 500000
                        # print("CHANGE TOOL!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
