...
const uint8_t spindleControl = 32;          // пин управления шпинделем: старт и стоп
...
uint32_t frameCounter = 0;              // счетчик выполненных к данному моменту кадров. Выводится на компьютер
                                        // для визуального контроля прогресса обработки

// флаги:
bool touchProbeIsOn = false;                // флаг показывает подключен ли в данный момент 3D Touch Probe
bool programInProgress = false;            // флаг статуса выполнения программы: когда программа начинает выполняться,
                                           // флагу programInProgress присваивается значение true. А когда в коде
                                           // встречается код M2 (конец программы) - programInProgress = false

bool xDir = false;                         // флаг хранит направление движения данной оси в данном кадре
                                           // true - вправо, false - влево
...
bool spindleIsOn = false;      // флаг хранит текущее состояние шпинделя: включен - выключен
...
class ToolChangePoint {
  ...
  uint8_t continueProgram() {
    ...
    aMove.toRiseSpindle();        // поднимаем шпиндель
    // включаем шпиндель и все дальнейшие перемещения происходят уже с включенным шпинделем
    if (spindleIsOn == false)
    {
        digitalWrite(spindleControl, HIGH);
        spindleIsOn = true;                    // установили флаг, что шпиндель запущен
        Serial.println("Spindle started");
        delay(5000);
    }
    else
    {
        Serial.println("During a tool change there was an attempt to turn on the spindle, but it is already running.");
    }
...
void setup() {
  ...
  pinMode(spindleControl, OUTPUT);      // пин управления шпинделем: старт и стоп
  ...
  digitalWrite(spindleControl, LOW);    // инициализируем шпиндель как выключенный вначале
}
...
uint8_t read_line_sd()
{
    ...
    while (true) {                  // поштучно читаем символы из файла
        ...
        else if (curChar == 'M')
        {
            ...
            while (true)
            {
                ...
                else        // все цифры M кода записаны. Дальше выясняем, какое число составлено из этих цифр
                {
                    ...
                    {
                        // во время автоматических перемещений, скорость подачи fSpeed будет изменяться. Поэтому
                        // мы значение переменной fSpeed запишем во временную переменную, чтобы потом перед тем как
                        // вернуться к выполнению программы, установить прежнее значение fSpeed
                        uint16_t fSpeedTemp = fSpeed;
                        ...
                        aMove.toRiseSpindle();          // поднимаем шпиндель
                        refPoint.goToRPoint();          // идем в точку G54
                    ...
                    else if (mTempInt == SPINDLE_START)
                    {
                        ...
                        {
                            Serial.println("There is an M3 code in the .nc file, but the spindle is already running.");
                        }
                    ...
                    else if (mTempInt == SPINDLE_STOP)
                    {
                        if (spindleIsOn == true)
                        {
                            ...
                            spindleIsOn = false;               // устанавливаем флаг, что шпиндель выключен
                            Serial.println("Spindle is stopped");
                        }
                        else
                        {
                            Serial.println("There is an M5 code in the .nc file, but the spindle is already stopped.");
                        }
        ...
        else if (curChar == '(')    // текущая строка - это комментарий
        {
            char textComment[70];      // текст комментария будет посимвольно сохнаняться в этом массиве
            uint16_t i = 0;             // номер ячейки массива textComment
            // читаем комментарий из файла посимвольно
            while(true)
            {
                // по сути - комментарий длится до закрывающей скобки, поэтому записываем в массив все символы кроме
                // закрывающей скобки.
                ...
                else        //достигнут конец комментария
                {
...
void startProgram()
{
    ...
    {
        programInProgress = true;     // ставим флаг статуса выполнения программы в true, что означает, что программа в
                                      // данный момент выполняется, а когда в .nc коде встретится код M2, статус
                                      // программы поменяется на "закончена" и programInProgress станет равен false
