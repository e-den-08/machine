...
//                                                 вычисляем:
    ...
    beatDur1Ax -= durHighLevel;                 // из длительности такта вычитаем длительность сигнала высокого уровня,
                                                // потому что он всегда неизменный и в вычислениях не участвует
  ...
  void sfpMoveX(uint16_t moveSpeedHigh, uint16_t moveSpeedLow) {     // метод вращает двигатель только по одной оси X
    if (!ovfX) {    // если таймер еще не переполнялся ни разу
        while (TCNT3 < moveSpeedLow * 2) {
            ...
            if (TCNT3 >= 65534) {break;}  // защита от зависания
        }
    }
    PORTE |= 1 << PORTE4;                 // подаем высокий уровень сигнала на первый мотор
    PORTE |= 1 << PORTE5;                 // подаем высокий уровень сигнала на второй мотор
...
void setup() {
  ...
  TIMSK4 = 0x00;
  TIMSK5 = 0x00;
  // разрешаем прерывания по переполнению и сравнению c регистром сравнения A (OCRnA)
  TIMSK3 |= (1 << TOIE3) | (1 << OCIE3A);
  TIMSK4 |= (1 << TOIE4) | (1 << OCIE4A);
  TIMSK5 |= (1 << TOIE3) | (1 << OCIE5A);
  // для справки:
