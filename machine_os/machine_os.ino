...
#define pinToolSensor 37        // пин, читает сенсор касания инструмента
...
class ToolChangePoint {
  ...
  void setToolSensorPoint() {
    // координаты датчика касания инструмента задаются тумблером pinSetToolSensor - пин 40
    // для установки датчика надо вручную подвести шпиндель над датчикот и назать тумблер pinSetToolSensor - движение в этой
...
void setup() {
  ...
  pinMode(pinToolSensor, INPUT);        // пин, включает режим ручного поиска прямоугольной заготовки снаружи с помощью датчика касания
    ...
    if (data[i] == 'F') {        // в строке нашли команду F - скорость подачи в мм/мин
      ...
      while (true) {                        // выясняем скорость подачи (считываем число из строки data по одной цифре)
        ...
        if (isNumber(data[i])) {                 // отыскиваем все цифры, следующие за буквой F
          ...
          speedSetting.setSpeed();          // вычислякм параметры движения для новой скорости
          break;                            // Выходим из цикла поиска всех цифр за буквой
        }
      }
    }
